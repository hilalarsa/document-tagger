...
for word in fullWords:
    ...
    for fullName in dictFromApi: #get each word from datastring

        singleNameArray = fullName.split() # get per name/rows from api
        # print(partName)
        ...
        for singleName in singleNameArray: #get per name word from splitted name/row from api
            ...
            if(seq.ratio() > 0.8): # if ratio is more than 80%, place data from api to output text
                ...
                outputText.append(singleName) # add singleName to 
                if(len(outputText) >= partLength):
                    result = outputText
                    outputText = []
                    print(result)
                    break
